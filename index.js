...
import {
  updateAllImageUrls,
  updateAllVideoUrls,
} from "./controllers/mediaControl.controller/mediaUpload.js";
...
app.put("/update-job-ads-keys", async (req, res) => {
  try {
    ...
    for (let jobAd of jobAds) {
      ...
      if (jobAd.applicationDeadline)
        jobAd.jobAdDeadline = jobAd.applicationDeadline;
...
const sendDynamicEmail = async (req, res) => {
  ...
  if (!toAddress || !toName || !subject || !body) {
    return res.status(400).json({
      error: "All fields (toAddress, toName, subject, body) are required",
    });
  }
